...
def fetch_posters(movie_id):
    url = f"https://api.themoviedb.org/3/movie/{movie_id}?api_key=544db60ca312982c71ffdb5fb1fa2736&language=en-US"
    print("Fetching poster from:", url)   # Debugging line

    try:
        response = requests.get(url, timeout=10)   # ✅ timeout added
        response.raise_for_status()                # ✅ catch bad responses (4xx/5xx)
...
def fetch_person_details(id_):
    data = requests.get(
        'https://api.themoviedb.org/3/person/{}?api_key=544db60ca312982c71ffdb5fb1fa2736'.format(id_)).json()
    print("Fetching person details from:", data)   # Debugging line
